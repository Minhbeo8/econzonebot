...
class RemoveMoney(commands.Cog):
    ...
    @is_bot_owner()
    async def remove_money(self, ctx, user: nextcord.Member, amount: int, currency_type: str = "ecoin"):
        amount = abs(amount)
        ...
        embed = nextcord.Embed(
            title=f"{Icons.success} Trừ tiền thành công", # SỬA ĐỔI
            description=f"Đã trừ `{amount}` {currency_icon} từ {user.mention}.",
